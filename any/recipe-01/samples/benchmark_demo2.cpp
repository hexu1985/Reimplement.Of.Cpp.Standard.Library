...
public:
    property(const std::string& name, const std::any& value)
...
int main() {
    ...
    std::cout <<
        std::any_cast<std::string>(properties[0].value()) << "\n";
    std::cout <<
        std::any_cast<int>(properties[1].value()) << "\n";
    std::cout <<
        std::any_cast<double>(properties[2].value()) << "\n";
}
