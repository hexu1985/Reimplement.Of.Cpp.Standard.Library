...
/*二，智能指针。 不能用any保存堆上的原始的指针，会造成内存泄露,应使用智能指针*/
    ...
    int* p = new int(2);
    std::any ptr_any(p);
    int* tmp = std::any_cast<int*>(ptr_any);
    PRINT(*tmp);

    //应当使用智能指针，而且只能使用shared_ptr,因为scoped_ptr不能被复制
    //这样在any析构时，会调用shared_ptr的析构函数，释放其持有的资源
    std::any shared_any(std::shared_ptr<int>(new int(3) ) );
    auto p_shared = std::any_cast<std::shared_ptr<int> >(shared_any);
    PRINT(*p_shared);
    puts("");
/**************************************************************************/
//三，辅助函数：
...
//四，用于容器:
    ...
    PRINT(std::any_cast<int>(vec[0]) );
    PRINT(std::any_cast<std::string&>(vec[1]) );
    PRINT(std::any_cast<double>(vec[2]) );
    PRINT(*std::any_cast<std::shared_ptr<std::string>>(vec[3]) );
