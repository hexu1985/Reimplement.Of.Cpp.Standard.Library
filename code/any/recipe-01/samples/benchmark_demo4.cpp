...
void foo(std::any& a) {
    ...
    try {
        std::shared_ptr<A> ptr=
            std::any_cast<std::shared_ptr<A> >(a);
    ...
    try {
        std::shared_ptr<B> ptr=
            std::any_cast<std::shared_ptr<B> >(a);
...
int main() {
    ...
    std::any a1(std::shared_ptr<A>(new A));
    std::any a2(std::string("Just a string"));

    {
        std::any b1(std::shared_ptr<A>(new B));
        std::any b2(std::shared_ptr<B>(new B));
